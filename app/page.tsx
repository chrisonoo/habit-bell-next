...
export default function HomePage() {
  ...
  const [isThirdSoundEnabled, setIsThirdSoundEnabled] = useState(() =>
    typeof window !== "undefined" && settings.isThirdSoundEnabled !== undefined
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      localStorage.setItem("habitBellSettings", JSON.stringify(settingsToSave))
    }
  }, [sessionDuration, minInterval, maxInterval, pause1Duration, pause2Duration, isThirdSoundEnabled])

  useEffect(() => {
    const initAudio = async () => {
      try {
        audioRefs.current = CONFIG.AUDIO_URLS.GONG1.map((url) => new Audio(url))
  ...
  const stopTraining = useCallback(() => {
    ...
    if (sessionTimerRef.current) clearTimeout(sessionTimerRef.current)
    if (countdownTimerRef.current) clearTimeout(countdownTimerRef.current)
    if (gongSequenceTimerRef.current) clearTimeout(gongSequenceTimerRef.current)
  }, [])

  const setNewInterval = useCallback(() => {
    const newInterval = Math.floor(Math.random() * (maxInterval - minInterval + 1) + minInterval)
  ...
  }, [isTraining, waitingForConfirmation, isLastInterval, stopTraining, setNewInterval])
      ...
      randomGong
        ...
        .then(() => {
          randomGong.onended = () => {
            if (isActiveRef.current) {
              setIsGongPlaying(false)
              gongSequenceTimerRef.current = setTimeout(playSecondGongSequence, pause1Duration * 1000)
            }
          }
        })
        .catch((error) => {
          ...
          .then(() => {
            audio2Ref.current!.onended = () => {
              if (isActiveRef.current) {
                ...
                count++
                if (count < 5) {
                  gongSequenceTimerRef.current = setTimeout(playNextGong, pause2Duration * 1000)
                } else {
  ...
  const playTestGong = useCallback(
    (gongNumber: 1 | 2 | 3) => {
      const audio = gongNumber === 1 ? getRandomGong1() : gongNumber === 2 ? audio2Ref.current : audio3Ref.current
      if (audio) {
        setIsGongPlaying(true)
        audio
          .play()
          ...
          .catch((error) => {
            console.error(`Test gong ${gongNumber} playback failed:`, error)
            setIsGongPlaying(false)
            if (gongNumber === 1) {
              setAudioFailed(true)
  ...
  const resetToDefaults = useCallback(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      localStorage.setItem("habitBellSettings", JSON.stringify(defaultSettings))
    }
  }, [])
        ...
        <CardContent className="space-y-6 p-6">
          <div className="text-center flex flex-col justify-center min-h-[7rem]">
            ...
            <div className="flex-1 flex items-center justify-center mt-2">
              <SessionTimeLeft isTraining={isTraining} sessionTimeLeft={sessionTimeLeft} formatTime={formatTime} />
            </div>
          </div>
          <TrainingButtons
            isTraining={isTraining}
