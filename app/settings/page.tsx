...
export default function SettingsPage() {
  ...
  const playTestGong = useCallback((gongNumber: 1 | 2 | 3) => {
    const audio =
      gongNumber === 1
        ? audioRefs.current[Math.floor(Math.random() * audioRefs.current.length)]
        : gongNumber === 2
          ? audio2Ref.current
          : audio3Ref.current
    if (audio) {
    ...
    <Card className="w-full max-w-[600px] mx-auto">
      ...
      <CardContent className="space-y-8">
        <SessionDurationSlider
          ...
          setSessionDuration={(value) => updateSetting("sessionDuration", value)}
          isTraining={false}
        />
        <IntervalRangeSlider
          minInterval={settings.minInterval}
          maxInterval={settings.maxInterval}
          setMinInterval={(value) => updateSetting("minInterval", value)}
          setMaxInterval={(value) => updateSetting("maxInterval", value)}
          isTraining={false}
        />
        <PauseDurationSlider
          id="pause1-duration"
        ...
        <ThirdSoundToggle
          ...
          setIsThirdSoundEnabled={(value) => updateSetting("isThirdSoundEnabled", value)}
          isTraining={false}
        />
        <TestGongs
