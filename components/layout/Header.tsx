  ...
  return (
    <header className="flex items-center justify-between p-4 border-b">
      ...
      <div className="flex items-center space-x-2">
        <Bell className={`h-8 w-8 text-primary animate-bell ${isRinging ? "ringing" : ""}`} />
        ...
        <SheetContent className="flex flex-col w-[250px] sm:w-[300px]">
          <div className="pt-8 flex-grow">
            <nav className="flex flex-col space-y-2">
              <Link href="/" passHref>
                <Button variant="ghost" className="justify-start text-base py-3" onClick={() => setIsOpen(false)}>
          ...
          <div className="border-t pt-4 mt-auto text-center">
            <p className="text-sm text-muted-foreground">© {new Date().getFullYear()} Habit Bell</p>
            <p className="text-xs text-muted-foreground mt-1">Inspired by Pavlovian conditioning</p>
          </div>
        </SheetContent>
      </Sheet>
    </header>
